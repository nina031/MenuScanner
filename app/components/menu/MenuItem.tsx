...
import { View, Text, TouchableOpacity, ScrollView, StatusBar, Modal, Dimensions, ColorValue } from 'react-native';
...
const MenuItemModal: React.FC<MenuItemModalProps> = ({ item, onClose }) => {
  ...
  const getDietInfo = (diet: string): { colors: readonly [ColorValue, ColorValue]; icon: string; label: string; } => {
    const dietInfo: Record<string, { colors: readonly [ColorValue, ColorValue]; icon: string; label: string; }> = {
      'végétarien': {
